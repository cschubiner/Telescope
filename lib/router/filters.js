  ...
  */
  ...
  canView: function () {
    if(!this.ready() || Meteor.loggingIn()){
      ...
      this.render(getTemplate('no_rights_rush'));
    } else {
      this.next();
    }
  },
...
Meteor.startup( function (){
  ...
  if(Meteor.isClient){
    ...
    Router.onBeforeAction(filters.canView, {except: ['post_submit', 'post_edit', 'atSignIn', 'atSignUp', 'atForgotPwd', 'atResetPwd', 'signOut']});
    ...
    Router.plugin('ensureSignedIn', {only: ['comment_edit']});
